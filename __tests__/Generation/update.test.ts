...
const updatedDoc = { _id: mockGenId, status: GenerationStatus.COMPLETED, audioUrl: "https://cdn.example.com/out.mp3" };
...
describe("PATCH /generations/:id (admin update)", () => {
  ...
  it("200 — admin can update status and result fields", async () => {
    const res = await request(app)
      ...
      .send({ status: GenerationStatus.COMPLETED, audioUrl: "https://cdn.example.com/out.mp3" });
