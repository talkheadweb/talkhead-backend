...
const { QueueJobModel, QueueUtil } = jest.requireMock("@/Config/queue") as {
  QueueJobModel: { findById: jest.Mock };
  QueueUtil    : { remove: jest.Mock };
};
...
describe("DELETE /queue/:id", () => {
  it("200 — cancels a pending job", async () => {
    QueueJobModel.findById = jest.fn().mockResolvedValue(makeDoc());
  ...
  it("409 — cannot cancel a processing job", async () => {
    QueueJobModel.findById = jest.fn().mockResolvedValue(makeDoc(QueueJobStatus.PROCESSING));
    const res = await request(app).delete(`${ENDPOINT}/${mockId}`).set("x-api-key", VALID_KEY);
    expect(res.status).toBe(409);
  });

  it("409 — cannot cancel a completed job", async () => {
    QueueJobModel.findById = jest.fn().mockResolvedValue(makeDoc(QueueJobStatus.COMPLETED));
    const res = await request(app).delete(`${ENDPOINT}/${mockId}`).set("x-api-key", VALID_KEY);
    expect(res.status).toBe(409);
  });

  it("409 — already cancelled", async () => {
    QueueJobModel.findById = jest.fn().mockResolvedValue(makeDoc(QueueJobStatus.CANCELLED));
    const res = await request(app).delete(`${ENDPOINT}/${mockId}`).set("x-api-key", VALID_KEY);
    expect(res.status).toBe(409);
  });

  it("404 — job not found", async () => {
    QueueJobModel.findById = jest.fn().mockResolvedValue(null);
    const res = await request(app).delete(`${ENDPOINT}/${mockId}`).set("x-api-key", VALID_KEY);
